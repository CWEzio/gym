...
class MAPursuerEvaderEnv(gym.Env):
    ...
    def step(self, action):
        ...
        dp1_x = 0.8*np.cos(pursuer_action[0])
        dp1_y = 0.8*np.sin(pursuer_action[0])
        # Pursuer N0.2
        p2 = np.array([p2_x, p2_y])
        dp2_x = 0.8*np.cos(pursuer_action[1])
        dp2_y = 0.8*np.sin(pursuer_action[1])
        # Evader
        e = np.array([e_x, e_y])
        de_x = 1.2*np.cos(evader_action)
        de_y = 1.2*np.sin(evader_action)
        ...
        if np.sqrt(np.sum(np.square(p_new - ob1))) < ob1_r:
            # print("Pursuer No.1 collision obstacle 1")
        ...
        if np.sqrt(np.sum(np.square(p_new - ob2))) < ob2_r:
            # print("Pursuer No.1 collision obstacle 2")
        ...
        if np.sqrt(np.sum(np.square(p_new - ob1))) < ob1_r:
            # print("Evader collision obstacle 1")
            circle = [ob1[0], ob1[1], ob1_r]
            temp = [0, 0]
        ...
        done = (np.sqrt(np.sum(np.square(p1-e))) <= 2 or np.sqrt(np.sum(np.square(p2-e))) <= 2) and self.steps < 0
        done = bool(done)
        ...
        if done == 0:
            ...
            evader_reward = 1.0
            pursuer_reward_1 = (-1)*distance_difference[0]*0.5
            pursuer_reward_2 = (-1)*distance_difference[1]*0.5
            self.steps -= 1
        elif self.steps_beyond_done is None:
            self.steps_beyond_done = 0
            self.steps = 10
            pursuer_reward_1 = 100.0
            pursuer_reward_2 = 100.0
            evader_reward = -100.0
        else:
            self.steps = 10
        ...
        if np.sqrt(np.sum(np.square(p1-ob1))) < \
                ob1_r - 1e-7 or np.sqrt(np.sum(np.square(p2-ob1))) < \
                ob1_r - 1e-7 or np.sqrt(np.sum(np.square(e-ob1))) < \
                ob1_r - 1e-7 or np.sqrt(np.sum(np.square(p1-ob2))) < \
                ob2_r - 1e-7 or np.sqrt(np.sum(np.square(p2-ob2))) < \
                ob2_r - 1e-7 or np.sqrt(np.sum(np.square(e-ob2))) < ob2_r - 1e-7:
            print("agent is in the obstacle!!")
            time.sleep(1)
    ...
    def reset(self):
        ...
        #evader_state[4] = 80
        #evader_state[5] = 97
        #pursuer_state[0][0] = 63
        #pursuer_state[0][1] = 80
        #pursuer_state[1][0] = 80
        #pursuer_state[1][1] = 63
        ...
        # print(evader_state[4], evader_state[5], pursuer_state[0][0], pursuer_state[0][1], pursuer_state[1][0], pursuer_state[1][1])
